...
result of comprehension will be $\nothing$ if $n = 0$, otherwise the result will be $42$. There
...
As with the monadic view, nondeterminism can be represented using sets. Here we have two primitive
operations: \textsf{fail} is a ``dead-end'' branch, and $\sqcup$ represents a choice between two
values.
\begin{center}
  \begin{tabular}{lll}
  ...
  \end{tabular}
\end{center}
Note the way we accomplish this. Our implementation of \textsf{run} lifts computations into
singleton sets, and our operators call the continuation on whatever values should be
considered---either nothing in the case of \textsf{fail} or both $x$ and $y$ in the case of $x
\sqcup y$.
