...
observes that comprehension syntax can be used with more than just lists---in fact it can be used
with any monad. One of the simplest monads is, called \textsf{Maybe} is defined as
\[ \textsf{data Maybe}~\alpha = \textsf{Nothing} \mid \textsf{Just}~\alpha \]
in languages like Haskell. We can write a ``\textsf{Maybe} comprehension'' like,
\[
...
predicate and ``\textsf{many}'' which applies a parser multiple times. An amazing amount of
...
\textsf{F}~\gamma$ and $g: \alpha \to \textsf{F}~\beta$, we can write a function
\[ f \circ_\textsf{F} g = \textsf{join}^\textsf{F} \circ \textsf{map}^\textsf{F}~f
\circ g \]
that composes them together while properly keeping track of $\textsf{F}$. This is useful for
